...
export default function AreaChart({
  ...
  return (
    <div className={cx("area-chart", className)}>
      <svg
      ...
      >
          ...
          return (
            <g key={tick}>
              <line
                ...
                y2={y}
              />
              <text className="area-chart__y-label" x={geometry.box.x - 12} y={y}>
                {formatTick(tick)}
              </text>
            </g>
          );
        ...
        <path className="area-chart__line" d={geometry.line} />
        ...
        {active && (
          <>
            <line
              className="area-chart__cursor"
              x1={active.x}
              x2={active.x}
              y1={geometry.box.y}
              y2={geometry.box.y + geometry.box.h}
            />
            <circle className="area-chart__dot" cx={active.x} cy={active.y} r="4.5" />
          </>
        )}

        <rect
          className="area-chart__hit"
          x={geometry.box.x}
          y={geometry.box.y}
      ...
      {active && activePoint && (
        <div
          className="area-chart__tip"
          style={{
            left: `${(active.x / VB_W) * 100}%`,
            top: `${((active.y - 16) / height) * 100}%`,
          }}
        >
          <div className="area-chart__tip-value">{formatValue(activePoint.value)}</div>
          <div className="area-chart__tip-label">{activePoint.label}</div>
        </div>
      )}
    </div>
  );
